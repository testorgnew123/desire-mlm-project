// Raise a scheduled demand -- POST /demands/:id/raise in docs/07-API.md.
// Thin; every real guard lives in packages/services/src/payment-plans.ts.
...
import {
  ...
  raiseDemand,
  type Demand,
} from "@desire/services/payment-plans";
...
export async function POST(
  ...
  try {
    const updated = await raiseDemand(db, {
      demandId,
    ...
    if (error instanceof DemandNotScheduledError) {
      return problemResponse({
        status: 409, type: "demand-not-scheduled", title: "Demand cannot be raised",
