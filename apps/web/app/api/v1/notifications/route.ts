// A user's own notifications -- GET /notifications. Own, userId-scoped;
// no permission check beyond authentication (a notification already
// targets exactly one user, so there is no scope split to resolve). Thin;
// the real logic lives in packages/services/src/notifications.ts's
// listNotifications.
import { getPrismaClient } from "@desire/db";
import { SessionInvalidError, validateSession } from "@desire/services/auth";
import { listNotifications, type Notification } from "@desire/services/notifications";
...
export async function GET(request: Request) {
  ...
  const notifications = await listNotifications(db, { userId: session.userId });
  return Response.json({ notifications: notifications.map(serialize) }, { status: 200 });
}
