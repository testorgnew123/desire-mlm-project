// Collections console -- GET /collections/console in docs/07-API.md
// ("Overdue buckets, filters"). Thin; every real guard and the scoping
// itself live in packages/services/src/collections-sweep.ts's
...
export async function GET(request: Request) {
  ...
  try {
    const rows = await getCollectionsConsole(db, {
      orgId: session.user.orgId,
      actorId: session.userId,
      projectId: projectId ?? undefined,
      associateId: associateId ?? undefined,
    });
    return Response.json({ demands: rows }, { status: 200 });
  } catch (error) {
