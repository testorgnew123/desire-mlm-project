// Reassign a lead -- POST /leads/:id/reassign in docs/07-API.md ("Manager
// only. Reason required"). Thin; every real guard (permission, scope,
// mandatory reason, the claim release+reclaim) lives in
...
export async function POST(
  ...
  if (typeof body.toAssociateId !== "string" || typeof body.reason !== "string") {
    return problemResponse({
      status: 400, type: "invalid-body", title: "Invalid request body",
      detail: "toAssociateId and reason are required strings.", instance: url.pathname,
    });
  }
  ...
  try {
    const updated = await reassignLead(db, {
      leadId,
      toAssociateId: body.toAssociateId,
      reason: body.reason,
    ...
    if (error instanceof ReassignReasonRequiredError) {
      return problemResponse({
        status: 422, type: "reassign-reason-required", title: "Reason required",
