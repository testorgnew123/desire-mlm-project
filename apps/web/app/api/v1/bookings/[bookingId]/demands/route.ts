// Read a booking's demand schedule -- GET /bookings/:id/demands in
// docs/07-API.md. Gated by booking.read, not a separate demand.read -- a
// demand schedule is a view of its booking's payment plan. Thin; every real
// guard lives in packages/services/src/payment-plans.ts.
...
import { getDemandsForBooking, type Demand } from "@desire/services/payment-plans";
...
function serialize(demand: Demand) {
  return {
    id: demand.id,
    bookingId: demand.bookingId,
    sequence: demand.sequence,
    milestoneRef: demand.milestoneRef,
    description: demand.description,
    amount: demand.amount.toString(),
    gstAmount: demand.gstAmount.toString(),
    dueDate: demand.dueDate.toISOString(),
    raisedAt: demand.raisedAt?.toISOString() ?? null,
    status: demand.status,
    waivedAt: demand.waivedAt?.toISOString() ?? null,
    waiveReason: demand.waiveReason,
  };
}
...
export async function GET(
  ...
  try {
    const demands = await getDemandsForBooking(db, { bookingId, orgId: session.user.orgId, actorId: session.userId });
    return Response.json({ demands: demands.map(serialize) }, { status: 200 });
  } catch (error) {
