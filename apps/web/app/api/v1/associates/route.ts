// List associates -- GET /associates in docs/07-API.md ("Scoped: own +
// downline unless privileged"). Thin; the real scoping lives in
// packages/services/src/associates.ts's listAssociates.
...
export async function GET(request: Request) {
  ...
  try {
    const associates = await listAssociates(db, { orgId: session.user.orgId, actorId: session.userId });
    return Response.json({ associates }, { status: 200 });
  } catch (error) {
