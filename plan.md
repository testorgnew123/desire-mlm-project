...
1. **Data residency** — **the database is now US-hosted (`aws-us-east-2`, Ohio) as of 2026-09-20**, moved from Singapore to sit next to the Netlify function, which is locked to Ohio on the free tier. That move was a performance fix (it removed ~200 ms from *every* database query — see [22-LOAD-TEST-RESULTS](docs/22-LOAD-TEST-RESULTS.md)) and it was made with the client's explicit approval, but it changes the answer to this question: the data is no longer merely offshore, it is in the United States. Still not urgent today — **no KYC data is stored yet**, current seed/dev data is synthetic — but the question is now "is *US*-hosted KYC acceptable, or is India residency contractually required?" If India residency is ever required, neither the database nor Netlify Blobs can satisfy it and the hosting choice has to be reopened entirely. *Before the first real KYC record is stored (Trigger T1, [21-TIER-LIMITS §8](docs/21-TIER-LIMITS.md)) — practically, before Phase 2's real customer/associate onboarding, and definitely before Phase 4.* ([ADR-0001](docs/adr/0001-nextjs-netlify-neon.md))
