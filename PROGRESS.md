...
| | |
|---|---|
| **Current phase** | Phase 0 — Foundation, 14/16 done. Only external-account-blocked tasks remain (Netlify+GitHub wiring). Phase 3's pure engine already built ahead of order — risk-first sequencing, see Decision log |
| **Started** | 2026-09-05 |
| **Target** | 18–22 weeks from start |
| **Hosting** | Hosted Neon (`ap-southeast-1`, Postgres 18.6) wired and migrated. Local Docker Postgres 18 kept for offline dev / concurrency tests. **Repo pushed** to `github.com/testorgnew123/desire-mlm-project`. No Netlify project yet — CLI logged in as an unconfirmed account, not proceeding until resolved |
| **Last updated** | 2026-09-05 |

| Phase | Tasks | Done | Gates | Status |
|---|:-:|:-:|:-:|---|
| 0 — Foundation | 16 | 14 | 1/1 | Blocked on GitHub push + Netlify (2 tasks remain) |
...
- [ ] Neon DB branch per PR, wired to the Netlify deploy preview — repo pushed and confirmed via API. Netlify account confirmed as the user's own (`kyleinnovates`, both accounts theirs). Site created (`desire-mlm-project`, project id `f17342ea-c3ef-4cee-8371-785042643009`), `netlify.toml` written for the pnpm-workspace monorepo, production env vars set (`DATABASE_URL`/`DATABASE_URL_UNPOOLED` → hosted Neon, fresh `PII_ENCRYPTION_KEY`/`AUTH_SECRET`/`JOB_TRIGGER_SECRET` generated for production, distinct from dev). **A local `netlify deploy --build` was attempted twice** (first attempt failed on a `publish` path bug in `netlify.toml`, fixed; committed as a separate fix commit). Second attempt reported "Deploy is live!" — **verified false by directly curling the live URL**: both `/` and `/api/health` return `502`, with a control-character-corrupted module path (`Cannot find package 'ar\taskappsweb'`) — the well-documented failure mode of building an OpenNext/serverless bundle on **Windows** (backslash path separators) for a **Linux** Lambda runtime. **Still blocked**: needs the one unavoidable manual step — connecting the GitHub repo to this Netlify site via their dashboard (installing Netlify's GitHub App on the `testorgnew123` org requires an org-admin browser click, not something completable headlessly) — so the actual production build runs on Netlify's Linux cloud builders instead of this Windows machine, which should resolve the 502 as a side effect. See PROGRESS.md decision log for exact steps handed to the user
...
- [ ] GitHub Actions cron secrets (`APP_BASE_URL`, `JOB_TRIGGER_SECRET`) + a manual `workflow_dispatch` proving a job endpoint responds — the GitHub side is now unblocked (`gh secret set` would work, repo admin confirmed), but there are two real dependencies left: no `/api/jobs/*` endpoint exists yet (Phase 1), and `APP_BASE_URL` needs an actual deployed URL, which needs Netlify — see the item above
