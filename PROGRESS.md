...
- [ ] Neon DB branch per PR (ephemeral per-PR database) -- **written and validated, one manual step short of working**: `.github/workflows/neon-preview-branches.yml` creates/resets a Neon branch per PR, generates the client, applies migrations to it, points that PR's Netlify preview at it via a branch-scoped env override, and deletes both on PR close. Originally drafted in a session outside this conversation and found to pass the SITE id where Netlify's API wants the ACCOUNT id -- confirmed by introspecting `netlify api setEnvVarValue` (required path vars: `account_id`, `key`). Rewritten to use `netlify env:set --context branch:<name>`, a supported path that resolves the account itself rather than a hand-derived API shape. **Blocked on:** `NETLIFY_AUTH_TOKEN` (Netlify has no API to mint a PAT -- manual UI step), and confirming `NEON_PROJECT_ID` points at *this* project (it was set in that other session and its comment referenced a different Neon org)
