...
| 2026-09-20 | Downline scope resolution stopped reading the whole table; the other half of the same plan item deliberately NOT done | `getAccessibleAssociateIds` (`packages/services/src/rbac.ts`) loaded **every** current hierarchy placement — no `orgId` filter, no bound — and narrowed it in memory, once per scoped query, on a model sized for 2,000 associates and hit by most list screens. Worth stating plainly: this was **never a data leak**, because `isInScope` frames its match as `/<cuid>/` and cuids are globally unique, so another org's row could not match — it was pure waste. Replaced with two indexed lookups (the actor's own placement, then a `startsWith` on the subtree path), which the `associate_hierarchy_path_prefix` index added the same day now serves. That also made the planned `orgId` parameter unnecessary — a descendant of the actor is in the actor's org by construction — so no call site changed, versus the ~10 the planned approach would have touched. Because this is row-scoping code it was not trusted on inspection: a new `rbac-scope.test.ts` builds a real seven-node tree and asserts the DB path returns exactly what the pure, already-well-tested `resolveAccessibleAssociateIds` returns **for every actor in it**, using the pure function as oracle rather than hand-written expectations, plus superseded-placement and no-placement cases. **Proven to actually fail**, the same discipline as the partial-index gate: removing the prefix filter turns 8 of the 11 red, including every per-actor equivalence check. The plan's other half — threading pre-resolved permissions into `getCollectionsConsole`/`listReceipts` to dedupe the repeated `userRole.findMany` — was **dropped on judgement**: each is one indexed query (~1–5 ms now the database is colocated), and the current shape, where every function resolves the actor's own roles, is safe by construction. Accepting a caller-supplied permission set on authorization-sensitive signatures would create a way to pass the *wrong* actor's permissions — a poor trade for milliseconds | Tech lead |
