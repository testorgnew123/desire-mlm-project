...
| **Hosting** | Hosted Neon (`ap-southeast-1`, Postgres 18.6) wired and migrated. Local Docker Postgres 18 kept for offline dev / concurrency tests. No Netlify project yet — pending GitHub push |
...
- [ ] Neon DB branch per PR, wired to the Netlify deploy preview — genuinely blocked: needs the repo pushed to GitHub and Netlify connected, which the user is doing after this block
...
- [ ] GitHub Actions cron secrets (`APP_BASE_URL`, `JOB_TRIGGER_SECRET`) + a manual `workflow_dispatch` proving a job endpoint responds — genuinely blocked: the workflow exists (`.github/workflows/scheduled-jobs.yml`) but no `/api/jobs/*` endpoint exists yet to trigger, and there's no GitHub remote yet to hold the secrets
