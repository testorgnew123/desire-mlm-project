...
| | |
|---|---|
| **Current phase** | **Phase 1 — Inventory is complete (17/17, both gates passed).** Hold concurrency proven against real Postgres with negative-control verification; the hold-taking endpoint and lost-race UI closed the last item, proven with a genuine concurrent HTTP race whose winner was confirmed independently in Postgres, not assumed. Phase 0 is also **complete** (17/17). **CI is green.** Phase 3's pure engine built ahead of order — risk-first sequencing, see Decision log. **Next: Phase 2 — Sales & Collections**, or the client answers blocking Phase 3 (see Blocked on client) |
| **Started** | 2026-09-05 |
